			...
			<div class="grid-content2 bg-purple-dark1">
				<div>
					<el-table :data="CustomerData" @selection-change="handleSelectionChange" stripe
						...
						<el-table-column prop="cbMoney" min-width="50" label="金额" align="center">
						</el-table-column>
						<el-table-column prop="cbState" min-width="50" label="收取状态" align="center" :formatter="cbStateFormat">
						</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			cbDeliverTypeFormat(row,column){
				if(row.cbDeliverType == 0){
					return '寄付'
				}else{
					return '到付'
				}
		...
		created() {
			var _this = this
			this.axios.get("http://localhost:8089/Logistics/fiCashBill/selectAll", {
				params: this.pageInfo
			})
			.then(function(response) {
				console.log(response)
