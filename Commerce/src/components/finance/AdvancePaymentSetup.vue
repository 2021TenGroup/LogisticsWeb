<template>
					...
					<el-table :data="CustomerData" @selection-change="handleSelectionChange" stripe
						...
						<el-table-column prop="acState" min-width="50" label="启用状态" align="center">
							<template v-slot="scope">
								{{scope.row.acState==0 && scope.row.acBalance>scope.row.acCloseBalance ?'启用':'未启用'}}
							</template>
						</el-table-column>
						<el-table-column label="操作" align="center">
							<template #default="scope">
								<el-button @click="showEdit(scope.row)" class="el-button--primary" size="small">设置
								</el-button>
								<el-button @click="showEdit1(scope.row)" class="el-button--primary2" size="small">查看
								</el-button>
								<!-- <el-button @click="delSelectAllCourseDetail(scope.row)" class="el-button--primary2"
									size="small">删除
								</el-button> -->
							</template>
						</el-table-column>
	...
	<!-- 弹窗 -->
	<el-dialog title="设置预付款信息" v-model="dialogVisible2" width="30%" :before-close="handleClose">
		<el-form :model="Customer" :rules="rules" ref="Customer" label-width="100px">
			...
			<el-form-item label="启用状态:">
				...
				<el-select v-model="Customer.acState" placeholder="请输入启用状态">
			...
			<el-form-item style="margin-right: 80px;" class="el-form-butt-show-one-s">
				<el-button type="primary" @click="updateCustomer()">立即提交</el-button>
			</el-form-item>
		</el-form>
	</el-dialog>
	<!-- 弹窗 -->
	<el-dialog title="查看预付款信息" v-model="dialogVisible1" width="30%" :before-close="handleClose">
		<el-form :model="Customer" :rules="rules" ref="Customer" label-width="100px">
			<el-row>
				<el-form-item label="当前金额:">
					<el-input v-model="Customer.acBalance" disabled style="width: 100px;"></el-input>
				</el-form-item>
				<el-form-item label="警戒金额:">
					<el-input v-model="Customer.acAlertBalance" disabled style="width: 100px;"></el-input>
				</el-form-item>
			</el-row>
			<el-row>
				<el-form-item label="关闭金额:">
					<el-input v-model="Customer.acCloseBalance" disabled style="width: 100px;"></el-input>
				</el-form-item>
				<el-form-item label="启用状态:">
					<el-select v-model="Customer.acState" disabled style="width: 100px;">
						<el-option v-for="item in uptheshow" :key="item.uid" :label="item.uname" :value="item.uid">
						</el-option>
					</el-select>
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			showEdit1(row) {
				this.Customer.acId = row.acId
				this.Customer.acBalance = row.acBalance
				this.Customer.acAlertBalance = row.acAlertBalance
				this.Customer.acCloseBalance = row.acCloseBalance
				this.Customer.acState = row.acState
